...
function showPendingTasks(tasks) {
    ...
    tasks.sort((a, b) => {
        ...
        if (sortDueDateValue !== 'none') {
            ...
            // If due dates are the same, use priority comparison
            if (dateComparison === 0) {
                // If no priority sorting is set, use default high-to-low priority
                if (sortPriorityValue === 'none') {
                    const priorityOrder = { high: 1, medium: 2, low: 3 };
                    priorityComparison = priorityOrder[a.priority] - priorityOrder[b.priority];
                }
        ...
        // If sorting by due date is not applied, sort by priority only
        if (sortPriorityValue === 'none') {
            const priorityOrder = { high: 1, medium: 2, low: 3 };
            priorityComparison = priorityOrder[a.priority] - priorityOrder[b.priority];
            return priorityComparison;
